%Removing atoms belong to the detected DeBaCl clusters from the dataset and then presenting the DeBaCl-detected cluster centers in the dataset 
function PlottingDeBaClClusterCentersInNoise(ConsideredAsNoise,DetectedPositionCenterSaver,ColorsForPlot,AxisLimits,IgnorePersistency,hdbscanPersistencyThreshold)
...
if dataset3D
    guidata(hfig,struct('Resolution',2,'xlimLL',xlimLL,'xlimUL',xlimUL,'ylimLL',ylimLL,'ylimUL',ylimUL,'zlimLL',zlimLL,'zlimUL',zlimUL,...
        'xlimLLs',xlimLL,'xlimULs',xlimUL,'ylimLLs',ylimLL,'ylimULs',ylimUL,'zlimLLs',zlimLL,'zlimULs',zlimUL));
else
    guidata(hfig,struct('Resolution',2,'xlimLL',xlimLL,'xlimUL',xlimUL,'ylimLL',ylimLL,'ylimUL',ylimUL,...
...
if IgnorePersistency
    saveas(hfig,'3_DeBaClClusterCentersInNoise_NOTconsideringPersistancy.tiff')
   % saveas(hfig,'3_DeBaClClusterCentersInNoise_NOTconsideringPersistancy.fig')
else
    saveas(hfig,'3_DeBaClClusterCentersInNoise_ConsideringPersistancy.tiff')
    %saveas(hfig,'3_DeBaClClusterCentersInNoise_ConsideringPersistancy.fig')
end
%close(hfig)
