...
function [DistGnrtdWOthers,gnrtdClusCenters,hdbAllClusCenters,hdbSelClusCenters,DaBaClClusCntrWithPersis,KNNClusCntrWithPersis]=PP1_01function_OnlyPersis(AxisLimits,DatasetName)
...
PP0Figh=figure(500);
NamesForPLot = categorical({'Generated','HDBSCAN_-All','HDBSCAN_-Selected','DeBaCl_-WithPersis'});
NamesForPLot =reordercats(NamesForPLot,{'Generated','HDBSCAN_-All','HDBSCAN_-Selected','DeBaCl_-WithPersis'});
ClusterSizeFOrPlot = [size(gnrtdClusCenters(:,1),1),size(hdbAllClusCenters(:,1),1),size(hdbSelClusCenters(:,1),1),size(DaBaClClusCntrWithPersis(:,1),1)];
...
for i=1:size(gnrtdClusCenters,1)
    ...
    Save=[Save;distgnrtd distHDB(1,1) distHDBsel(1,1) distDBCwPersis(1,1)];
end
...
group = [ ones(size(Save(:,1)));...
         ...
         3 * ones(size(Save(:,3)));...
         4*ones(size(Save(:,4)));...
         5*ones(size(dbcWdbcWithPersis(:,1)))];
boxplot([Save(:,1); Save(:,2); Save(:,3);Save(:,4);dbcWdbcWithPersis(:,1)],group)
ylim([0 inf])
set(gca,'XTickLabel',{'Generated','HDBSCAN_All','HDBSCAN_Selected','DeBaCl_WPersis','DeBaClWDeBaCl_WPersis'},'YMinorTick','on')
...
group1=[ones(size(gnrtdClusCenters(:,4)));...
       ...
       4*ones(size(DaBaClClusCntrWithPersis(:,4)));...
       5*ones(size(KNNClusCntrWithPersis(:,4)))];
boxplot([gnrtdClusCenters(:,4); hdbAllClusCenters(:,9); hdbSelClusCenters(:,9); DaBaClClusCntrWithPersis(:,4);KNNClusCntrWithPersis(:,4)],group1)
ylim([0 inf])
set(gca,'XTickLabel',{'Generated','HDBSCAN_All','HDBSCAN_Selected','DeBaCl_WithPersistancy','KNN_WithPersistancy'},'YMinorTick','on')
